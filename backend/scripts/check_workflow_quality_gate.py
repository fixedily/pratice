...
DEFAULT_GATE_CONFIG = ROOT / "evaluation" / "workflow_quality_gate.json"
...
def run_gate(gate_path: Path, result_path: Path | None) -> int:
    ...
    result_file_cfg = str(gate_cfg.get("result_file") or "evaluation/workflow_eval_results.json")
    ...
    print(f"[workflow-quality-gate] 评测文件: {eval_path}")
    print(f"[workflow-quality-gate] 门禁配置: {gate_path.resolve()}")
    for line in passed:
        print(f"[PASS] {line}")
    if failed:
        for line in failed:
            print(f"[FAIL] {line}")
        print(f"[workflow-quality-gate] 门禁失败，共 {len(failed)} 项不达标。")
        return 1

    print("[workflow-quality-gate] 门禁通过，工作流质量自动化评测达标。")
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="检查工作流质量评测结果门禁")
    parser.add_argument(
        "--gate-config",
        default=str(DEFAULT_GATE_CONFIG),
        help="门禁配置 JSON 路径（默认 backend/evaluation/workflow_quality_gate.json）",
    )
    parser.add_argument(
        "--result-file",
        default=None,
